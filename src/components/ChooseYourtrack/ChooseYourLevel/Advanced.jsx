...
function Advanced({ savedQuestions, setSavedQuestions }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://questionprep.azurewebsites.net/api/Questions/Q_AdvancedLevel/${frameworkId}`);
        ...
        <div className="container">
            <div className="flex justify-between">
                <div className="flex items-center justify-center gap-3">
                    ...
                    <h1>{AdvancedQuestions[0]?.frameworkName}</h1>
                    <h1>{AdvancedQuestions[0]?.levelName}</h1>
                </div>
